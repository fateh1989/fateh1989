...
class YmTikTokAccessibilityService : AccessibilityService() {
    ...
    private var composerGestureUsed = false
    private var sendGestureSide = 0
    ...
    private val feedDebounce = object : Runnable {
        override fun run() {
            ...
            if (anyTikTokRootHasPanel()) return
            beginComment("new_video")
        }
    }
    ...
    override fun onServiceConnected() {
        ...
        instance = this
        reloadFromPrefs()
        record("engine_ready", "محرك زر 4 متعدد النوافذ جاهز")
    }
    ...
    fun reloadFromPrefs() {
        ...
        if (!wasEnabled) {
            record("auto_on", "زر 4 يعمل: فتح ثم كتابة ثم نشر ثم إغلاق")
            handler.postDelayed({
    ...
    private fun openComments(attempt: Int) {
        ...
        if (clickNode(candidate)) {
            panelOpened = true
            record("comment_opened", "تم فتح التعليقات؛ الانتقال إلى خانة الكتابة")
        ...
        if (attempt < 5) {
            ...
            fail("comment_button_missing", "تعذر العثور على زر التعليقات")
        }
    }

    private fun activateComposer(attempt: Int) {
        if (!enabled) {
            fail("disabled", "تم إيقاف زر 4")
            return
        }
        ...
        if (editor != null) {
            focusEditor(editor)
            record("editor_found", "تم العثور على محرر التعليق")
            handler.postDelayed({ writeComment(0) }, 160L)
            return
        }

        val entry = bestNodeAcrossTikTok(::composerEntryScore, 8)
        if (entry != null && attempt <= 7) {
            val clicked = clickNode(entry)
            record(
                if (clicked) "composer_clicked" else "composer_click_retry",
                if (clicked) "تم ضغط خانة إضافة تعليق" else "وجدت خانة التعليق وأعيد محاولة ضغطها",
            )
            handler.postDelayed({ activateComposer(attempt + 1) }, 300L)
            return
        }

        if (!composerGestureUsed && attempt >= 2) {
            composerGestureUsed = true
            val dm = resources.displayMetrics
            record("composer_tap", "نقر مباشر على خانة التعليق")
            dispatchTap(dm.widthPixels * 0.50f, dm.heightPixels * 0.88f) {
        ...
        if (attempt < 14) {
            ...
            fail("editor_missing", "فتح YM التعليقات لكن لم يجد محرر الكتابة")
        }
    }
    ...
    private fun writeComment(attempt: Int) {
        ...
        if (editor == null) {
            if (attempt < 5) {
                ...
                fail("editor_lost", "اختفى محرر التعليق قبل الكتابة")
            }
            return
        }
        ...
        val direct = editor.performAction(AccessibilityNodeInfo.ACTION_SET_TEXT, args)
        if (direct) {
            record("text_set", "تم إرسال النص إلى محرر TikTok مباشرة")
            handler.postDelayed({ verifyOrSend(0) }, 260L)
            return
        }

        if (pasteInto(editor)) {
            record("text_paste", "تم إدخال التعليق بالنسخ واللصق")
            handler.postDelayed({ verifyOrSend(0) }, 300L)
            return
        }

        if (attempt < 4) {
            record("write_retry", "إعادة محاولة إدخال التعليق ${attempt + 1}/5")
            handler.postDelayed({ writeComment(attempt + 1) }, 240L)
        } else {
            fail("write_failed", "TikTok رفض SET_TEXT واللصق")
        }
    }

    private fun pasteInto(editor: AccessibilityNodeInfo): Boolean {
        return runCatching {
    ...
    private fun verifyOrSend(attempt: Int) {
        val editor = findEditorAcrossTikTok()
        if (editor == null) {
            if (attempt < 4) {
                handler.postDelayed({ verifyOrSend(attempt + 1) }, 220L)
            } else {
                // Some TikTok builds hide the editor node after accepting text.
                record("text_node_hidden", "محرر النص غير ظاهر؛ متابعة البحث عن زر النشر")
                findAndSend(0)
            }
            return
        }

        val actual = editor.text?.toString().orEmpty().trim()
        val expected = pendingText.trim()
        if (actual == expected || (expected.isNotEmpty() && actual.contains(expected))) {
            record("text_confirmed", "تم تأكيد التعليق داخل الحقل")
            findAndSend(0)
            return
        }
        ...
        if (attempt < 2) {
            ...
            // Do not get stuck forever on builds that do not expose EditText text back to accessibility.
            record("text_unreadable", "تعذر قراءة النص من TikTok؛ محاولة النشر")
            findAndSend(0)
        }
    }
    ...
    private fun findAndSend(attempt: Int) {
        val editor = findEditorAcrossTikTok()
        val send = if (editor != null) {
            findSendNearEditor(editor)
        ...
        if (attempt >= 2 && sendGestureSide < 2) {
            val dm = resources.displayMetrics
            val y = editor?.let {
                Rect().also(it::getBoundsInScreen).centerY().toFloat()
            }?.takeIf { it > 0f } ?: (dm.heightPixels * 0.86f)
            val x = if (sendGestureSide == 0) dm.widthPixels * 0.93f else dm.widthPixels * 0.07f
            sendGestureSide += 1
            record("send_tap", "نقر مباشر على موضع زر النشر")
            dispatchTap(x, y) {
        ...
        if (attempt < 8) {
            ...
            fail("send_missing", "تعذر العثور على زر نشر التعليق")
        }
    }
    ...
    private fun verifySubmitted(attempt: Int) {
        ...
        if (sendGestureSide < 2) {
            findAndSend(2)
        } else {
            fail("send_unconfirmed", "النص بقي في حقل التعليق بعد محاولة النشر")
        }
    }
    ...
    private fun succeed() {
        ...
        record("comment_ok", "تم نشر التعليق؛ إغلاق لوحة المفاتيح والتعليقات")
    ...
    private fun closeAfterAttempt(success: Boolean, backCount: Int) {
        ...
        if (backCount > 0) {
            ...
            if (roots.isNotEmpty() && roots.none(::isPanelLikelyOpen)) {
                record(
                    ...
                    if (success) "تم نشر التعليق وإغلاق نافذة التعليقات" else "تم إغلاق نافذة التعليقات بعد فشل المحاولة",
                )
                resetFlight()
                return
            }
        ...
        // First BACK may close the keyboard; second BACK closes the TikTok comments sheet.
        if (backCount >= 2) {
            record("panel_close_finished", "انتهت محاولتا إغلاق لوحة المفاتيح/التعليقات")
            resetFlight()
            return
        }

        performGlobalAction(GLOBAL_ACTION_BACK)
        record("closing_panel", "رجوع ${backCount + 1}/2 لإغلاق لوحة المفاتيح/التعليقات")
    ...
    private fun findEditorAcrossTikTok(): AccessibilityNodeInfo? {
        tiktokRoots().forEach { root ->
            ...
            if (focused != null && editorScore(focused) >= 10) return focused
        }
        return bestNodeAcrossTikTok(::editorScore, 14)
    }
    ...
    private fun isPanelLikelyOpen(root: AccessibilityNodeInfo): Boolean {
        if (findBestNode(root, ::editorScore, 14) != null) return true
        if (findBestNode(root, ::composerEntryScore, 8) != null) return true
        var evidence = 0
        walk(root, 450) { node ->
            val token = token(node)
            if (containsAny(
                    token,
                    "comments", "comment list", "reply", "replies",
                    "kommentarer", "kommentar", "svara",
                    "التعليقات", "تعليقات", "رد",
                )) evidence++
        }
        return evidence >= 3
    }
    ...
    private fun commentButtonScore(node: AccessibilityNodeInfo): Int {
        ...
        if (token.contains("view comments") || token.contains("open comments")) score += 38
        ...
        if (containsAny(token, "add comment", "add a comment", "write a comment", "comment here", "اكتب تعليق", "أضف تعليق")) score -= 26
        return score
    }
    ...
    private fun composerEntryScore(node: AccessibilityNodeInfo): Int {
        ...
        if (containsAny(
                token,
                "add comment", "add a comment", "write a comment", "write comment", "comment here", "say something",
                "kommentera", "skriv en kommentar", "lägg till kommentar",
                "أضف تعليق", "اكتب تعليق",
            )) score += 34
    ...
    private fun editorScore(node: AccessibilityNodeInfo): Int {
        ...
        val token = token(node)
        var score = 0
        if (node.isEditable) score += 34
        if (node.className?.toString()?.contains("EditText", true) == true) score += 22
        if (supports(node, AccessibilityNodeInfo.ACTION_SET_TEXT)) score += 18
    ...
    private fun sendScore(node: AccessibilityNodeInfo): Int {
        ...
        var score = 0
        if (containsAny(token, "send_comment", "post_comment", "submit_comment", "comment_send", "comment_post")) score += 44
        if (label in setOf("send", "post", "publish", "submit", "skicka", "publicera", "إرسال", "نشر")) score += 34
        if (containsAny(token, "send comment", "post comment", "publish comment", "submit comment", "skicka", "publicera", "إرسال", "نشر")) score += 18
    ...
    private fun dispatchTap(x: Float, y: Float, after: () -> Unit) {
        ...
        val gesture = GestureDescription.Builder()
            .addStroke(GestureDescription.StrokeDescription(path, 0L, 75L))
            .build()
