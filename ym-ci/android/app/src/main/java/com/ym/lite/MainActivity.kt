...
class MainActivity : AppCompatActivity() {
    ...
    private fun bindFeedControls() {
        feedVideo.setOnPreparedListener { mediaPlayer -> mediaPlayer.isLooping = true; feedVideo.start() }
        feedVideo.setOnTouchListener { _, event ->
            when (event.actionMasked) {
                MotionEvent.ACTION_DOWN -> { touchDownY = event.y; true }
                MotionEvent.ACTION_UP -> {
                    val delta = event.y - touchDownY
                    if (abs(delta) > 100f) { if (delta < 0) nextVideo() else previousVideo() }
                    else { if (feedVideo.isPlaying) feedVideo.pause() else feedVideo.start() }
                    true
                }
                else -> true
            }
        ...
        findViewById<TextView>(R.id.bubbleAuto).setOnClickListener { openPanel(R.id.panelAuto, "AUTO") }
        ...
        findViewById<TextView>(R.id.bubbleHistory).setOnClickListener { openPanel(R.id.panelHistory, "السجل"); checkLastPost() }
        ...
        autoRailButton.setOnClickListener { openPanel(R.id.panelAuto, "AUTO") }

        findViewById<TextView>(R.id.homeNav).setOnClickListener { closeSheet() }
        findViewById<TextView>(R.id.historyNav).setOnClickListener { openPanel(R.id.panelHistory, "السجل"); checkLastPost() }
        findViewById<TextView>(R.id.profileNav).setOnClickListener { openPanel(R.id.panelAccounts, "أنا / الحسابات") }
        findViewById<TextView>(R.id.discoverNav).setOnClickListener { Toast.makeText(this, "اكتشف", Toast.LENGTH_SHORT).show() }
        ...
        findViewById<TextView>(R.id.commentButton).setOnClickListener { openPanel(R.id.panelAuto, "AUTO / التعليقات") }
        ...
        findViewById<TextView>(R.id.followingTab).setOnClickListener { Toast.makeText(this, "يتابع", Toast.LENGTH_SHORT).show() }
    }

    private fun bindAutomationControls() {
        findViewById<Button>(R.id.openAccessibility).setOnClickListener { startActivity(Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS)) }
        findViewById<Button>(R.id.saveAutoSettings).setOnClickListener { saveAutoSettings(autoPrefs.getBoolean("auto_enabled", false)); setStatus("تم حفظ إعدادات AUTO") }
    ...
    private fun saveAutoSettings(masterEnabled: Boolean) {
        val interval = scrollInterval.text.toString().toIntOrNull()?.coerceIn(3, 120) ?: 8
    ...
    private fun stopAuto() {
        autoPrefs.edit().putBoolean("auto_enabled", false).putBoolean("pending_launch", false).apply()
        YmAccessibilityService.notifyConfigChanged()
        updateAutoUi()
    ...
    private fun updateAutoUi() {
        ...
        val running = autoPrefs.getBoolean("auto_enabled", false)
        autoServiceState.text = "خدمة الوصول: ${if (access) "مفعلة" else "غير مفعلة"} • AUTO: ${if (running) "يعمل" else "متوقف"}"
        autoRailButton.text = if (running) "AUTO\n●" else "AUTO\n○"
    }
    ...
    private fun openPanel(panelId: Int, title: String) {
        intArrayOf(R.id.panelAuto, R.id.panelWheel, R.id.panelSchedule, R.id.panelAccounts, R.id.panelHistory, R.id.panelConnection)
    ...
    private fun playCurrentVideo() {
        if (selected.isEmpty()) { feedPlaceholder.visibility = View.VISIBLE; return }
    ...
    private fun nextVideo() { if (selected.isNotEmpty()) { currentFeedIndex = (currentFeedIndex + 1) % selected.size; playCurrentVideo() } }
    private fun previousVideo() { if (selected.isNotEmpty()) { currentFeedIndex = if (currentFeedIndex == 0) selected.lastIndex else currentFeedIndex - 1; playCurrentVideo() } }
    ...
    private fun shareCurrentVideo() {
        val uri = selected.getOrNull(currentFeedIndex) ?: run { Toast.makeText(this, "اختر فيديو أولًا", Toast.LENGTH_SHORT).show(); return }
    ...
    private fun client(): YmRelayClient {
        val url = prefs.workerUrl(); val token = prefs.token()
    ...
    private fun runTask(block: () -> Unit) {
        if (!busy.compareAndSet(false, true)) { setStatus("هناك عملية جارية الآن"); return }
        progress.visibility = View.VISIBLE
        executor.execute {
            try { block() } catch (e: Exception) { runOnUiThread { setStatus("خطأ: ${e.message}") } }
    ...
    private fun connectTikTok() = runTask {
        ...
        runOnUiThread { startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(url))); setStatus("بعد إنهاء الربط ارجع إلى YM واضغط تحديث الحسابات") }
    }
    ...
    private fun scheduleProofPost() {
        try {
            val account = currentAccount(); require(selected.isNotEmpty()) { "اختر فيديوًا واحدًا على الأقل" }
            val uri = selected.first(); val text = caption.text.toString()
            runTask {
                val api = client(); val upload = api.createUploadTarget()
                ...
                runOnUiThread { setStatus("تمت جدولة منشور الاختبار بعد 10 دقائق. رقم العملية: $postId") }
            }
    ...
    private fun checkLastPost() {
        ...
        if (postId.isBlank()) { setStatus("لا يوجد منشور اختبار محفوظ بعد"); return }
        runTask {
            val results = client().getPostResults(postId)
            runOnUiThread {
                if (results.isEmpty()) setStatus("لم تظهر نتيجة نهائية بعد للمنشور $postId")
                else {
                    val success = results.firstOrNull { it.success }
                    if (success != null) {
                        localPrefs.edit().putBoolean("provider_proof_passed", true).apply()
                        setStatus("نجح اختبار النشر لدى المزود. ${success.url ?: "الرابط غير متاح بعد"}\nتم فتح جدولة الشهر في YM.")
                    } else setStatus("فشل اختبار النشر: ${results.firstNotNullOfOrNull { it.error } ?: "بدون رسالة واضحة"}")
                }
            }
        }
    }
    ...
    private fun createCloudPlan() {
        try {
            require(localPrefs.getBoolean("provider_proof_passed", false)) { "نفّذ اختبار منشور واحد أولًا وتحقق من نجاحه قبل إنشاء خطة شهرية" }
            val account = currentAccount(); require(selected.isNotEmpty()) { "اختر فيديوهات الدولاب" }
            ...
            val plan = PlanEngine.generate(ZoneId.systemDefault(), LocalDate.now().plusDays(1), days, perDay, start, end, selected.size, account.id.hashCode().toLong())
            val text = caption.text.toString(); progress.max = plan.size; progress.progress = 0
            setStatus("سيتم إنشاء ${plan.size} منشورًا سحابيًا. لا تغلق التطبيق أثناء رفع الخطة.")
            runTask {
                val api = client()
                plan.forEachIndexed { index, slot ->
                    val uri = selected[slot.mediaIndex]; val upload = api.createUploadTarget()
                    api.uploadSigned(contentResolver, uri, upload.uploadUrl, contentResolver.getType(uri))
                    api.createScheduledPost(account.id, text, upload.mediaUrl, slot.instant.toString())
                    runOnUiThread { progress.progress = index + 1; status.text = "${index + 1}/${plan.size} — ${slot.instant}" }
                }
                runOnUiThread { setStatus("تمت جدولة ${plan.size} منشورًا في السحابة. يمكنك الآن إطفاء الهاتف.") }
            }
        } catch (e: Exception) { setStatus("خطأ: ${e.message}") }
    }
    ...
    override fun onPause() { super.onPause(); if (feedVideo.isPlaying) feedVideo.pause() }
    ...
    override fun onDestroy() { super.onDestroy(); executor.shutdownNow() }
}
