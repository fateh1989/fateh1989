...
'''    private val scopeWatch = object : Runnable {\n        override fun run() {\n            localPrefs.edit().putBoolean("diag_service_connected", true)\n                .putLong("diag_service_heartbeat", System.currentTimeMillis()).apply()\n            if (isTikTokForeground()) ensureOverlay() else removeOverlay()\n''',
"service heartbeat")

s = replace_once(s,
'''    override fun onServiceConnected() {\n        super.onServiceConnected()\n        instance = this\n        prefs.edit().putBoolean("comment_mode_active", false).apply()\n''',
'''    override fun onServiceConnected() {\n        super.onServiceConnected()\n        instance = this\n        val now = System.currentTimeMillis()\n        localPrefs.edit().putBoolean("diag_service_connected", true)\n            .putLong("diag_service_connected_at", now)\n            .putLong("diag_service_heartbeat", now)\n            .putBoolean("diag_overlay_visible", false)\n            .remove("diag_overlay_error").apply()\n        prefs.edit().putBoolean("comment_mode_active", false).apply()\n''',
"service connected diagnostics")

s = replace_once(s,
'''    override fun onAccessibilityEvent(event: AccessibilityEvent?) {\n        val packageName = event?.packageName\n''',
'''    override fun onAccessibilityEvent(event: AccessibilityEvent?) {\n        val packageName = event?.packageName\n        localPrefs.edit().putString("diag_last_event_package", packageName?.toString().orEmpty())\n            .putLong("diag_last_event_at", System.currentTimeMillis()).apply()\n''',
"event diagnostics")

s = replace_once(s,
'''        if (instance === this) instance = null\n        super.onDestroy()\n''',
'''        localPrefs.edit().putBoolean("diag_service_connected", false)\n            .putBoolean("diag_overlay_visible", false).apply()\n        if (instance === this) instance = null\n        super.onDestroy()\n''',
"destroy diagnostics")

s = replace_once(s,
'''        wm.addView(container, lp)\n        overlayContainer = container\n''',
'''        localPrefs.edit().putLong("diag_overlay_attempt_at", System.currentTimeMillis()).apply()\n        try {\n            wm.addView(container, lp)\n        } catch (t: Throwable) {\n            localPrefs.edit().putBoolean("diag_overlay_visible", false)\n                .putString("diag_overlay_error", "${t::class.java.simpleName}: ${t.message.orEmpty()}")\n                .putLong("diag_overlay_error_at", System.currentTimeMillis()).apply()\n            Toast.makeText(this, "YM overlay error: ${t::class.java.simpleName}", Toast.LENGTH_LONG).show()\n            return\n        }\n        localPrefs.edit().putBoolean("diag_overlay_visible", true)\n            .remove("diag_overlay_error").apply()\n        overlayContainer = container\n''',
