...
|---|---|---|---|---|
| RUN | `fateh1989/1122` | `main` | Active Android automation / WhatsApp scheduling app. Existing codebase includes scheduling, multi-recipient work, attachments, retries/history, templates, local AI, wheel automation, Accessibility execution, auto-reply, drip campaigns, backup, and a debug-only Test Lab. A protected rollback snapshot exists as branch `run-golden-191`. Stable builds now use a monotonic high Android `versionCode`, CI verifies the embedded APK version, and `ci/verify_run_baseline.py` blocks silent removal of core features/components/tests. | Continue only from current `main`; never present an older branch/APK or reduced feature set as a newer RUN release. Stable APKs come from `main`. Preserve `run-golden-191` as comparison/recovery only. Every new stable version must contain the previous stable version plus the intended change. Build/test before claiming success; real-device behavior still requires device QA. |
