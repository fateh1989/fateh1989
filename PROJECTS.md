...
| YM / يم | `YM.md` in this repository; current source/state in ChatGPT Library `/Projects/YM` | N/A | Active **small Android** TikTok scheduling project for up to 3 accounts. v0.6 uses `Android -> tiny Cloudflare Worker relay -> Post for Me Quickstart -> TikTok`. Worker tests are **8/8**, the Kotlin month planner compiles/smoke-passes, Android unit tests pass, and the first debug APK built successfully in GitHub Actions run `35046059180`. Real-device installation and real TikTok posting are not verified yet. | Read `YM.md`, then `/Projects/YM/STATE.md`, then inspect `YM-Lite-v0.6.zip` and the compiled `YM-v0.6-debug.apk`. Keep the app small. Do not deploy a VPS by default. Next: real-device smoke QA, connect Post for Me Quickstart + Worker secrets, then prove one scheduled public post with the phone off before expanding to 30-day/3-account use. |
