...
| YM / يم | `YM.md` in this repository; current bootstrap in ChatGPT Library `/Projects/YM` | N/A | Active TikTok growth/publishing project for up to 3 accounts. Core v0.3 has persistent wheels/jobs, server-side worker, OAuth v2 flow, encrypted token storage/refresh, official Direct Post provider code, and async publish-status reconciliation. Verified locally with 15 tests and smoke startup; no real TikTok credential or real post has been verified yet. | Read `YM.md` first, then `/Projects/YM/STATE.md` and inspect the latest bootstrap. Do not merge YM into RUN. Preserve the 3-account limit and phone-off/server-side execution goal. Next: media delivery suitable for `PULL_FROM_URL`, always-on deployment, then first controlled real account/post verification. |
