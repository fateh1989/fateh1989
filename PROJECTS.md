...
| YM / يم | ChatGPT Library: `/Projects/YM` (interim bootstrap; no GitHub repo yet) | N/A | Separate TikTok growth/publishing project. Reuses RUN-style mechanics: wheels, time windows, quantity, history and no-repeat. Product limit: up to 3 TikTok accounts. Core v0.2 now persists accounts/timezones, wheel definitions/items, media references and durable publication jobs; it plans deterministic daily slots, survives backend restarts, prevents stale backlog bursts, and runs a background worker outside the phone. Current provider is still mock; no live TikTok credentials are wired. | Continue from `/Projects/YM/STATE.md` and the latest `YM-bootstrap.zip`. Do not merge YM into RUN. Preserve the 3-account limit and the phone-off/server-side execution goal. Real TikTok publishing must be wired through a supported/approved integration path and tested before claiming live operation. |
...
3. For the Android bridge, read `BRIDGE.md` before giving connection/restart instructions.
4. For heavy-equipment parts research, read `RESEARCH.md` and then verify the web live.
5. Never equate a blocked implementation path with an impossible project; preserve the goal and change the architecture/path when necessary.
6. Whenever a new project is created, moved, renamed, paused, or abandoned, update this registry immediately.
