...
| YM / يم | `YM.md` in this repository; current source/state in ChatGPT Library `/Projects/YM` | N/A | Active **small Android** TikTok scheduling project for up to 3 accounts. v0.6 pivots from the earlier self-hosted server to `Android -> tiny Cloudflare Worker relay -> Post for Me Quickstart -> TikTok`. Android/relay source is built, Worker tests are 6/6 and the pure Kotlin month planner compiles/smoke-passes. APK and real TikTok posting are not verified yet. | Read `YM.md`, then `/Projects/YM/STATE.md`, then inspect the latest `YM-Lite-v0.6.zip` or newer bundle. Keep the app small. Do not deploy a VPS by default. Next: compile APK, connect Post for Me Quickstart + Worker secrets, then prove one scheduled post with the phone off before expanding to 30-day/3-account use. |
