...
| YM / يم | `YM.md` in this repository; current bootstrap in ChatGPT Library `/Projects/YM` | N/A | Active TikTok growth/publishing project for up to 3 accounts. Core v0.5 has persistent wheels/jobs, background worker, OAuth/token handling, provider abstraction, local media delivery, ffprobe preflight, deployment readiness checks, and production Docker/Compose handoff. Verified locally with 25 tests plus real generated-video preflight/API smoke; no real TikTok credential or public post has been verified yet. | Read `YM.md` first, then `/Projects/YM/STATE.md` and inspect the latest bootstrap. Do not merge YM into RUN. Preserve the 3-account limit and phone-off/server-side execution goal. Next: deploy in mock mode on a persistent HTTPS host, verify restart persistence, then resolve TikTok's current audit/UX constraints before attempting account 1 live. |
