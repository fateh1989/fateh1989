...
| YM / يم | ChatGPT Library: `/Projects/YM` (interim bootstrap; no GitHub repo yet) | N/A | Separate TikTok growth/publishing project. Reuses RUN-style mechanics: wheels, time windows, quantity, history and no-repeat. Product limit: up to 3 TikTok accounts. Execution core is intended to run outside the phone so the phone may be fully powered off. Initial backend bootstrap is stored as `YM-bootstrap.zip` with `README.md` and `STATE.md`; current bootstrap uses a provider abstraction and mock publisher, not live TikTok credentials. | Continue from `/Projects/YM/STATE.md` and the latest bootstrap. Do not merge YM into RUN. Preserve the 3-account limit and the phone-off/server-side execution goal. Real TikTok publishing must be wired through a supported/approved integration path and tested before claiming live operation. |
