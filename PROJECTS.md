...
| YM / يم | `YM.md` in this repository; current state/APKs in ChatGPT Library `/Projects/YM`; current UI source branch `ym-ui-v0.7` | `ym-ui-v0.7` for current Android UI | Active small Android TikTok scheduling project for up to 3 accounts. v0.7 locks the product as a **video-first short-form feed** rather than a RUN-like control dashboard. The center `+` opens six bubbles: wheel, schedule, accounts, proof, history and settings. Existing Worker/planner tests pass; v0.7 Android unit tests and APK build passed in GitHub Actions run `35047608412`. v0.6 install/launch was verified on the real device; v0.7 visual device QA and real TikTok publishing are still pending. | Read `YM.md`, then `/Projects/YM/STATE.md`, then inspect `/Projects/YM/YM-v0.7-debug.apk` and branch `ym-ui-v0.7`. Preserve the video-first UX and six-bubble `+` interaction. Keep the app small. Next: install/visually QA v0.7, then connect Post for Me Quickstart + Worker secrets and prove one scheduled public post with the phone off before 30-day/3-account use. |
